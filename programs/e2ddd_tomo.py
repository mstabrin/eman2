...
def main():
    ...
    parser.add_header(name="orblock1", help='Just a visual separation', title="Corrections", row=2, col=0, rowspan=1, colspan=1, mode="align")

    parser.add_argument("--dark",type=str,default="",help="Perform dark image correction using the specified image file",guitype='filebox',browser="EMMovieRefsTable(withmodal=True,multiselect=True)", row=4, col=0, rowspan=1, colspan=3, mode="align,tomo,refs")
    parser.add_argument("--gain",type=str,default="",help="Perform gain image correction using the specified image file",guitype='filebox',browser="EMMovieRefsTable(withmodal=True,multiselect=True)", row=6, col=0, rowspan=1, colspan=3, mode="align,tomo,refs")

    parser.add_argument("--ref_label",type=str,default="",help="Optional: Specify a label for the averaged dark and gain references when using multiple, individual frames.\nA labeled will be written as movierefs/dark_<label>.hdf and movierefs/gain_<label>.hdf.\nNote: This option is ignored when using a single reference image/stack.",guitype='strbox', row=2, col=1, rowspan=1, colspan=2, mode="refs")
    parser.add_argument("--rotate_dark",  default = "0", type=str, choices=["0","90","180","270"], help="Rotate dark reference by 0, 90, 180, or 270 degrees. Default is 0. Transformation order is rotate then reverse.",guitype='combobox', choicelist='["0","90","180","270"]', row=5, col=0, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--reverse_dark", default=False, help="Flip dark reference along y axis. Default is False. Transformation order is rotate then reverse.",action="store_true",guitype='boolbox', row=5, col=1, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--k2", default=False, help="Perform gain image correction on gain images from a Gatan K2. Note, these are the reciprocal of typical DDD gain images.",action="store_true",guitype='boolbox', row=8, col=0, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--rotate_gain", default = 0, type=str, choices=["0","90","180","270"], help="Rotate gain reference by 0, 90, 180, or 270 degrees. Default is 0. Transformation order is rotate then reverse.",guitype='combobox', choicelist='["0","90","180","270"]', row=8, col=1, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--reverse_gain", default=False, help="Flip gain reference along y axis (about x axis). Default is False. Transformation order is rotate then reverse.",action="store_true",guitype='boolbox', row=8, col=2, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--de64", default=False, help="Perform gain image correction on DE64 data. Note, these should not be normalized.",action="store_true",guitype='boolbox', row=9, col=0, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--gain_darkcorrected", default=False, help="Do not dark correct gain image. False by default.",action="store_true",guitype='boolbox', row=9, col=1, rowspan=1, colspan=1, mode="refs")
    parser.add_argument("--invert_gain", default=False, help="Use reciprocal of input gain image",action="store_true",guitype='boolbox', row=9, col=2, rowspan=1, colspan=1, mode="refs")
    ...
    parser.add_header(name="orblock4", help='Just a visual separation', title="Alignment: ", row=10, col=0, rowspan=2, colspan=1, mode="align,tomo")
    ...
    parser.add_argument("--realign", action="store_true",help="Align frames using previous alignment parameters.",default=False, guitype='boolbox', row=11, col=2, rowspan=1, colspan=1, mode='align[False]')
    ...
    parser.add_header(name="orblock6", help='Just a visual separation', title="Optimization: ", row=24, col=0, rowspan=2, colspan=3, mode="align,tomo")

    parser.add_argument("--optccf",default="robust",type=str, choices=["robust","centerofmass","ccfmax"],help="Use this approach to determine relative frame translations.\nNote: 'robust' utilizes a bimodal Gaussian to robustly determine CCF peaks between pairs of frames in the presence of a fixed background.", guitype='combobox', row=18, col=0, rowspan=1, colspan=2, mode="align,tomo",choicelist='["robust","centerofmass","ccfmax"]')
    ...
    parser.add_argument("--threads", default=4,type=int,help="Number of threads to run in parallel. The default is 4, and our alignment routine requires 2+ threads. Using more threads will result in faster processing times.", guitype='intbox', row=20, col=0, rowspan=1, colspan=2, mode="align,tomo")
...
def process_movie(fsp,dark,gain,first,flast,step,options):
    ...
    if options.noali:
        mgdirname = os.path.join(".","micrographs")
